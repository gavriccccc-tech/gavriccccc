...
namespace InventoryTrackerApp
{
    public partial class MainForm : Form
    {
        ...
        private void CreateSidebar()
        {
            ...
            CreateNavButton(navPanel, "🗑️ Управление сделками", new EventHandler((s, e) => ShowManageTransactionsForm()));
            CreateNavButton(navPanel, "📊 Управление ордерами", new EventHandler((s, e) => ShowManageOrdersForm())); // НОВАЯ КНОПКА
            ...
            CreateNavButton(navPanel, "🖼️ Управление кэшем изображений", new EventHandler((s, e) => ShowImageCacheManagement()));

            // Кнопка выхода
            CreateNavButton(navPanel, "🚪 Выход", new EventHandler((s, e) => this.Close()));
        }
        ...
        private void ShowDashboard()
        {
            ...
            var orders = dataService.GetOrders(); // Получаем ордера для статистики
            ...
            // Карточка 4: Активные ордера
            int activeOrders = orders.Count(o => o.Status == "Активный");
            CreateStatCard(startX + (cardWidth + spacing) * 3, startY, cardWidth, cardHeight,
                "📊 Активные ордера", activeOrders.ToString(),
                Color.FromArgb(155, 89, 182), "Требуют внимания");
            ...
            if (gamesStats.Count > 0)
            {
                ...
                // Статистика по ордерам (если есть активные)
                if (activeOrders > 0)
                {
                    Label ordersStatsLabel = new Label();
                    ordersStatsLabel.Text = "📊 Активные ордера";
                    ordersStatsLabel.Font = new Font("Segoe UI", 16, FontStyle.Bold);
                    ordersStatsLabel.ForeColor = DarkColor;
                    ordersStatsLabel.Size = new Size(300, 40);
                    ordersStatsLabel.Location = new Point(900, 280);
                    contentPanel.Controls.Add(ordersStatsLabel);

                    Panel ordersPanel = new Panel();
                    ordersPanel.Location = new Point(900, 330);
                    ordersPanel.Size = new Size(400, 150);
                    ...
                    var activeOrdersList = orders.Where(o => o.Status == "Активный").Take(3).ToList();
                    int orderY = 10;
                    foreach (var order in activeOrdersList)
                    {
                        Label orderLabel = new Label();
                        orderLabel.Text = $"{order.Item}: {order.FilledQuantity}/{order.TargetQuantity} шт. ({order.ProgressPercent:0}%)";
                        orderLabel.Font = new Font("Segoe UI", 9);
                        orderLabel.ForeColor = order.ProgressPercent > 50 ? Color.Green : Color.Orange;
                        orderLabel.Location = new Point(10, orderY);
                        orderLabel.AutoSize = true;
                        ordersPanel.Controls.Add(orderLabel);
                        orderY += 25;
                    }

                    if (activeOrders > 3)
                    {
                        Label moreOrdersLabel = new Label();
                        moreOrdersLabel.Text = $"... и еще {activeOrders - 3} ордеров";
                        moreOrdersLabel.Font = new Font("Segoe UI", 8, FontStyle.Italic);
                        moreOrdersLabel.ForeColor = Color.Gray;
                        moreOrdersLabel.Location = new Point(10, orderY);
                        moreOrdersLabel.AutoSize = true;
                        ordersPanel.Controls.Add(moreOrdersLabel);
                    }
                }
                ...
                CreateQuickActionButton(440, 550, "📊 Новый ордер", new EventHandler((s, e) => ShowAddOrderForm()));
                CreateQuickActionButton(600, 550, "📥 Импорт CSV", new EventHandler((s, e) => ShowImportCSVForm()));
                CreateQuickActionButton(760, 550, "🔄 Цены Steam", new EventHandler((s, e) => RefreshSteamPrices()));
            }
            else
            {
                ...
                CreateQuickActionButton(440, 330, "📊 Новый ордер", new EventHandler((s, e) => ShowAddOrderForm()));
                CreateQuickActionButton(600, 330, "📥 Импорт CSV", new EventHandler((s, e) => ShowImportCSVForm()));
                CreateQuickActionButton(760, 330, "🔄 Цены Steam", new EventHandler((s, e) => RefreshSteamPrices()));
            }
        }
        ...
        private string GetStatsText()
        {
            ...
            var activeOrders = orders.Count(o => o.Status == "Активный");

            return $"📊 Статистика: {stats.TotalTransactions} сделок | {gamesCount} игр | {totalItems} предметов | {activeOrders} активных ордеров | Прибыль: {stats.TotalProfit:0.00} руб.";
        }
        ...
        // НОВЫЙ МЕТОД: Показать форму управления ордерами
        private void ShowManageOrdersForm()
        {
            using (var form = new ManageOrdersForm(dataService))
            {
                form.ShowDialog();
                RefreshStats();
                ShowDashboard();
        ...
        // НОВЫЙ МЕТОД: Показать форму добавления ордера
